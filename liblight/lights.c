        ...
        struct light_state_t const* state)
{
    ...
    // Asus X00T RG Led driver use only 255(0xFF) colors
    if (colorARGB > 0xFF000000 && state == &g_battery) {
        if (red >= green) {
            green = 0;
            red = 0xFF;
        } else {
    ...
    } else if (colorARGB > 0xFF000000 && state == &g_notification) {
        green = 0xFF;
        red = 0;
    } else {
        green = 0;
    ...
    if (state == &g_battery)
        ledMode = 1;
    else if (state == &g_notification)
        ledMode = 2;
    else
        ledMode = 0;

    ALOGD("set_speaker_light_locked mode=%d, ledMode=%d, colorARGB=%08X, onMS=%d, offMS=%d, blink=%d, red=%d, green=%d",
        state->flashMode, ledMode, colorARGB, onMS, offMS, blink, red, green);
#endif

    if (blink) {
        if (green) {
            write_int(GREEN_BLINK_FILE, blink); // green blink for notifications only
        } 
        if (red) {
            write_int(RED_BLINK_FILE, blink); // red blink for battery only
        }
    } else {
